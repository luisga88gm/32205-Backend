...
class ProductMananger {
  ...
  addProduct = async (title, description, price, thumbnail, stock, code) => {
    if ((title, description, price, thumbnail, stock, code)) {
      if (fs.existsSync(this.path)) {
        ...
        if (codeCheck) {
          console.log(
            `Código existente, por favor agregue un producto valido.`
          );
        } else {
          if (result.length > 0) {
      ...
      console.log(`Por favor, agregue todos los detalles del producto`);
    }
  };
  getProduct = async () => {
    if (fs.existsSync(this.path)) {
  ...
  uptadeProduct = async (id, propiedadActualizadas) => {
    if ((id, propiedadActualizadas)) {
      ...
      console.log(`Completa todas los propiedades para actualizar el producto`);
    }
  };
  deleteProduct = async (id) => {
    if (fs.existsSync(this.path)) {
      if (id) {
        ...
        console.log(`Por favor, coloca el ID para eliminar el producto`);
      }
    } else {
      console.log(
